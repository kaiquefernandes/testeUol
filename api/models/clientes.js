...
exports.insertClient = async (ctx) => {
  const { nome, data_nascimento, sexo, idade, cidade } = ctx;
  let query = `INSERT INTO CLIENTES VALUES ("", "${nome}", "${moment(
    data_nascimento,
    "DD/MM/YYYY"
  ).format("YYYY-MM-DD")}", "${sexo}", ${idade}, "${cidade}", "${moment(
    new Date()
